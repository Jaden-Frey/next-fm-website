...
export default function AdminProductModal({ isOpen, onClose, onSubmit, initialData }: ModalProps) {
  // Default category is lowercase 'beef' to match your database expectations
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
        setFormData({
            ...initialData,
            // Ensure category is lowercase when editing
            category: initialData.category.toLowerCase() 
        });
      } else {
        // Reset form for new product
        setFormData({ name: "", price: 0, description: "", category: "beef", image: "", onSale: false });
      }
      setError(null);
    }
  }, [initialData, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Ensure strict lowercase before sending
      await onSubmit({
        ...formData,
          ...
          <div className="modal-content border-0 shadow-lg overflow-hidden rounded-4">
            ...
            <form onSubmit={handleSubmit}>
                  ...
                  <div className="col-lg-7">
                    <div className="row g-3">
                      ...
                      <div className="col-md-6">
                        <label className="form-label fw-bold small text-uppercase text-secondary">Price</label>
                        <div className="input-group">
                           <span className="input-group-text bg-light border-end-0 fw-bold text-dark">R</span>
                           <input type="number" className="form-control border-start-0 ps-1" placeholder="0.00" required value={formData.price} onChange={(e) => setFormData({...formData, price: Number(e.target.value)})} />
                        </div>
                      </div>
              ...
              <div className="modal-footer bg-white border-top-0 p-4">
                ...
                <button 
                  type="submit" 
                  className="btn btn-dark px-5 rounded-pill shadow fw-bold d-flex align-items-center gap-2" 
