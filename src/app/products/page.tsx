...
function ProductsContent() {
  ...
  const handleDelete = async (mongoId: string) => {
    ...
    const res = await fetch(`/api/products?id=${mongoId}`, {
      method: "DELETE",
    });
  ...
  // Delete entire category (admin only)
  const handleDeleteCategory = async () => {
    if (!isAdmin) return;
    if (selectedCategory === "all") {
      alert("Cannot delete 'All Products'. Select a specific category first.");
      return;
    }

    if (
      !confirm(
        `Delete ALL products in category "${selectedCategory}"? This cannot be undone.`
      )
    )
      return;
  ...
  return (
    <>
            ...
            {CATEGORY_DEFINITIONS.map((cat) => (
              <button
                ...
                className={`btn px-4 py-2 border-0 fw-bold ${
                  selectedCategory === cat.id ? "btn-dark" : "btn-light bg-white text-muted"
                }`}
              >
                {cat.name}
                <span
                  className={`badge ms-2 rounded-pill ${
                    selectedCategory === cat.id ? "bg-white text-dark" : "bg-light text-dark border"
                  }`}
                >
          ...
          <div className="row row-cols-1 row-cols-md-3 row-cols-xl-4 g-4">
            ...
            {filteredProducts.map((product) => (
              <div key={product.id} className="col">
                <div className="card h-100 shadow-sm border-0 transition-transform hover-lift position-relative overflow-hidden group">
                  {isAdmin && (
                  ...
                  {product.onSale && (
                    <div className="badge bg-danger text-white position-absolute top-0 start-0 m-2" style={{ zIndex: 1 }}>
                      SALE
                    </div>
                  )}
                  ...
                  <div className="card-body text-center d-flex flex-column">
                    ...
                    <div className="mb-3 mt-auto">
                      <span className={`fw-bold ${product.onSale ? "text-danger fs-5" : "fs-5 text-dark"}`}>R{product.price.toFixed(2)}</span>
                    </div>
                  </div>
      ...
      <AdminProductModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={editingProduct ? handleUpdate : handleCreate} initialData={editingProduct} />
